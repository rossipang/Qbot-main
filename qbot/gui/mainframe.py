...
from qbot.gui.panels.panel_strategy_chat import StrategyChatPanel
...
class MainFrame(wx.Frame):
    def __init__(self, *args, **kw):
        ...
        # 设置程序图标（相对本文件定位，缺失则跳过，避免启动报错）
        icon_file = Path(__file__).resolve().parent / "imgs" / "logo.ico"
        if icon_file.is_file():
            try:
                self.SetIcon(wx.Icon(str(icon_file), wx.BITMAP_TYPE_ICO))
            except Exception:
                pass
    ...
    def init_main_tabs(self):
        ...
        self.tabs.AddPage(StrategyChatPanel(self.tabs), "Cursor 策略编写", True)
        ...
        # 今日盯盘：人选定的几只，盘中准实时刷价和买点
        self.tabs.AddPage(IntradayWatchPanel(self.tabs), "今日盯盘", True)
