...
def fetch_forward_news(
    finance_limit: int = 30, tech_limit: int = 40, pharma_limit: int = 20, *, fast: bool = False
) -> pd.DataFrame:
    """
    ...
    _TECH_KEEP = (
        ...
        # 培育钻石/金刚石热沉：栏目常不带「芯片」字，必须单独放行
        "培育钻石", "人造钻石", "金刚石", "热沉", "散热材料", "金刚石散热",
        "力量钻石", "黄河旋风", "四方达",
    )
    _PHARMA_KEEP = (
    ...
    # —— 快讯通道（财联社+见闻全球/A股）：近一周实时催化，不过科技白名单 ——
    # 戴尔财报/美股服务器订单等常在这里；若塞进 tech_rows 会被 _TECH_KEEP 滤掉
    ...
    out = out.sort_values(["_ord", "_major"], ascending=[False, False]).drop(
        columns=["_major", "_ord"]
    )
